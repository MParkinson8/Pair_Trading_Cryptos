...
#the following computes the cointegration parameter for the time series supplied
#by doing simple OLS regression of coin2 over [1, coin1]
    def calculate_beta(self,coin1,coin2):

        Y = coin2
        X = sm.add_constant(coin1)
        #b = np.linalg.inv(X.T.dot(X)).dot(X.T).dot(Y)  #the inversion is inefficient
        model=sm.OLS(Y,X)
        results=model.fit()
...
#the following computes the spread and its normalisation, only present value is returned
